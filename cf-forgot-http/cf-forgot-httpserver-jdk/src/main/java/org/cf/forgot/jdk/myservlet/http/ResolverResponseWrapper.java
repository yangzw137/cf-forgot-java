...
/**
 * Provides a convenient implementation of the ResolverResponse interface that
 * can be subclassed by developers wishing to adapt the response from a Resolver.
 * This class implements the Wrapper or Decorator pattern. Methods default to
 * calling through to the wrapped response object.
 * 
 * @author Various
 * @since v 2.3
 * @see ResolverResponse
 */
public class ResolverResponseWrapper implements ResolverResponse {
    private ResolverResponse response;

    /**
     * Creates a ResolverResponse adaptor wrapping the given response object.
     * 
     * @throws IllegalArgumentException
     *             if the response is null.
     */
    public ResolverResponseWrapper(ResolverResponse response) {
        if (response == null) {
    ...
    /**
     * Return the wrapped ResolverResponse object.
     */
    public ResolverResponse getResponse() {
     ...
     *             if the response is null.
     */
    public void setResponse(ResolverResponse response) {
        if (response == null) {
            throw new IllegalArgumentException("Response cannot be null");
        }
        this.response = response;
    ...
    /**
     * @param wrapped
     * @since Resolver 3.0 TODO SERVLET3 - Add comments
     */
    public boolean isWrapperFor(ResolverResponse wrapped) {
        if (response == wrapped) {
            return true;
        }
        if (response instanceof ResolverResponseWrapper) {
            return ((ResolverResponseWrapper) response).isWrapperFor(wrapped);
        }
        return false;
    }
    ...
    public boolean isWrapperFor(@SuppressWarnings("rawtypes") Class wrappedType) {
        ...
        if (response instanceof ResolverResponseWrapper) {
            return ((ResolverResponseWrapper) response).isWrapperFor(wrappedType);
        }
        return false;
    }
