...
/**
 * Provides a convenient implementation of the ResolverRequest interface that can
 * be subclassed by developers wishing to adapt the request to a Resolver. This
 ...
 * @see ResolverRequest
 */
public class ResolverRequestWrapper implements ResolverRequest {
    private ResolverRequest request;

    /**
     * Creates a ResolverRequest adaptor wrapping the given request object.
     * 
     * @throws IllegalArgumentException if the request is null
     */
    public ResolverRequestWrapper(ResolverRequest request) {
        if (request == null) {
     ...
     * @throws IllegalArgumentException if the request is null.
     */
    public void setRequest(ResolverRequest request) {
        if (request == null) {
            throw new IllegalArgumentException("Request cannot be null");
        }
        this.request = request;
    ...
    /**
     * The default behavior of this method is to return getResolverContext() on
     * the wrapped request object.
     * 
     * @since Resolver 3.0
     */
...
//        return request.getResolverContext();
//    }
...
//    }
     ...
     * the wrapped request object.
     * 
     * @param servletRequest    The ResolverRequest with which to initialise the
     *                          asynchronous context
     * @param servletResponse   The ResolverResponse with which to initialise the
     *                          asynchronous context
     * @throws IllegalStateException If asynchronous processing is not supported
     *         for this request or if the request is already in asynchronous
     *         mode
     ...
     */
//    @Override
//    public AsyncContext startAsync(ResolverRequest servletRequest,
//            ResolverResponse servletResponse) throws IllegalStateException {
...
//    }
     ...
     * @since Resolver 3.0
     */
    public boolean isWrapperFor(ResolverRequest wrapped) {
        if (request == wrapped) {
            return true;
        }
        if (request instanceof ResolverRequestWrapper) {
            return ((ResolverRequestWrapper) request).isWrapperFor(wrapped);
        }
        return false;
    }
    ...
    public boolean isWrapperFor(@SuppressWarnings("rawtypes") Class wrappedType) {
        ...
        if (request instanceof ResolverRequestWrapper) {
            return ((ResolverRequestWrapper) request).isWrapperFor(wrappedType);
        }
        return false;
    }
